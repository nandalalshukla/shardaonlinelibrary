...
export const searchNotes = async (req: Request, res: Response) => {
  try {
    ...
    // Query parameter is now optional, if not provided return filtered notes
    ...
    const filterConditions: any = {
      status: "approved", // Only search approved notes
    };
    ...
    const notes = await Note.find(filterConditions)
      .sort({ createdAt: -1 })
